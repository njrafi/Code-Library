...
i64 triArea2(const point &a, const point &b, const point &c)
{
...
struct circle
{
    ...
    **/
    ...
    long double areacircle(circle v)
    {
        ...
        long double theta1 = acos((r*r + d*d - v.r*v.r) / (2.0*r*d));
        long double a1 = theta1*r*r;
        long double theta2 = acos((v.r*v.r + d*d - r*r) / (2.0*v.r*d));
        long double a2 = theta2*v.r*v.r;

        long double triangle = r*r*sin(theta1*2) / 2 + v.r*v.r*sin(theta2*2) / 2;
...
struct polygon
{
    ...
    **/
    ...
    // polygon er segment er upore thakleo true return kore
    bool pointinconvexpolygon(point q)
    {
        i64 st = 1, en = n-1, mid;
        while(en - st > 1)
        {
            mid = (st + en)>>1;
            if(triArea2(p[0], p[mid], q) < 0) en = mid;
            else st = mid;
        }
        if(triArea2(p[0], p[st], q) < 0) return false;
        if(triArea2(p[st], p[en], q) < 0) return false;
        if(triArea2(p[en], p[0], q) < 0) return false;
        return true;
    }
